...
export default function Home() {
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    // Connect to metamask
    // Since we store `web3Modal` as a reference, we need access the `current` value to get access to the underlying  object 
  ...
  const getNumberOfWhitelisted = async () => {
    try {
      ...
      const _numAddressesWhitelisted = await whitelistContract.numWhitelistedAddresses();
  ...
  const checkIfAddressInWhitelist = async () => {
    try {
      // we will need a signer later to get the user's signer
